...
```{r make-season-pal}
make_season_pal <- function(season_num, n_clusters = 8) {
  s_num <- season_num
  ...
  pal_name <- quickpalette::url_palette(url = poster_url, n_clusters = n_clusters)
  c(pal_name)
}
```

Cool, now I should just do this, right? `r emo::ji("smirk")`
...
Let's get organised here, and put this in a data frame:

```{r archer-frame}
archer_seasons <- data_frame(
